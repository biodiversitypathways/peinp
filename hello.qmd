...
site_covariates <- pei_main %>% 
  select(location) %>% 
  unique() %>% 
  mutate(coast_dist = runif(n(), 10, 673)) %>% 
  arrange(location)
...
recs_plot <- cirrus_recs |>
  rename("Month" = month) |>
  rename("Location" = location) |>
  ggplot(aes(x = Month, y = Location, fill = Location)) +
  geom_density_ridges() +
  facet_wrap(~ year) +
  theme_minimal() +
...
verified_tags |> DT::datatable(rownames=F, filter="top")
```
...
occu_one <- wt_format_occupancy(pei_occu, siteCovs = site_covariates, species = "BTBW")
occu_model <- occu(~ doy + hr + observer ~ 1, occu_one)
occ_fit <- predict(occu_model, data = site_covariates, type="det")
occ_graph <- occ_fit %>%
  as_tibble() %>%
  bind_cols(., occu_one@obsCovs) %>%
  arrange(doy)

ggplot(occ_graph, aes(x = doy, y = Predicted)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Predicted Detection Probabilities with Confidence Intervals",
       x = "Day of year",

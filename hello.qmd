...
occu_one <- wt_format_occupancy(pei_occu, siteCovs = site_covariates, species = "OVEN")
...
ggplot(occ_graph %>% arrange(location), aes(x = location, y = Predicted)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  labs(title = "Predicted occupancy at locations with confidence intervals",
       x = "Location",
...
my_qpad_model <- lm(OVEN ~ 1, offset=offstz$OVEN, data=pei_wide)

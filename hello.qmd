...
occu_one <- wt_format_occupancy(pei_occu, siteCovs = site_covariates, species = "NOPA")
...
occ_graph_all <- bind_rows(occ_graph2019, occ_graph2020, occ_graph2021, occ_graph2022, occ_graph2023) %>%
  mutate(year = factor(year(recording_date_time))) %>%
  select(Predicted:upper,year)

ggplot(occ_graph, aes(x = year, y = Predicted, group = year, fill=year)) +
  geom_boxplot(alpha = 0.3) +
  labs(title = "Predicted Change Across Years with Boxplot and Line",
       x = "Year",
       y = "Predicted") +
  theme_bw()
```

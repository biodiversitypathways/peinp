...
bans_projects <- wt_get_projects('ARU') |> filter(grepl('Bank Swallow Monitoring 2025|Bank Swallow Monitoring 2023', project)) |> pull(project_id)
...
#| collapse: true
#| code-fold: true
#| tbl-cap: Summary of audio data processed for Bank Swallow at each location

bans_locs <- c("BANS-BOW-E","BANS-BOW-N","BANS-BOW-W","PENP-BS-6")

tbl_bans_analyze <- bans_aru |>
  filter(task_is_complete == TRUE) |>
  group_by(location) |>
  summarise(sum_analyzed = round((sum(task_duration)/3600),2))

total_bans <- all_recs |>
  mutate(year = year(recording_date_time)) |>
  filter(location %in% bans_locs) |>
  group_by(location, year) |>
  summarise(sum = round((sum(recording_duration)/3600),2)) |>
  left_join(tbl_bans_analyze, by = "location") |>
  mutate(proportion_analyzed = round(((sum_analyzed / sum)*100),2))

datatable(total_bans)

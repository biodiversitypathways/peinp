...
# Create a new R project and version control from the repo
# Pull from main
# Load the pei.RData file
# Render
...
#betadiver(raw_dog[,4:103])
#diversity(raw_dog[,4:103], index="shannon")
```
...
For testing species occupancy, we selected locations with a minimum of four dawn visits across five years, focusing on forest obligate species for ecological relevance (see @tbl-bird-guilds). Utilizing a single-season single-species occupancy model (see @fig-spp-occ) from Mackenzie et al. 2003. Site-specific covariates included the distance to the ocean edge and the proportion of forested area surrounding the ARU. Observation covariates incorporated day of the year, hour, observer details, and a quadratic term for both year and hour. Despite variations in methodology between 2019 (1SPM) and subsequent years (2020 - 2023), we maintained consistency by exclusively utilizing the time to the first detection of individuals from the 1SPM recordings. Model predictions were then generated, and the best model was selected based on the Aikaike Information Criterion (AIC).
...
site_covariates_19 <- pei_occu_19 |>
  inner_join(distance_to_coast) |>
  select(location, year, coast_dist) |>
  distinct()
...
occu_model_19 <- occu(~ doy + hr + observer ~ coast_dist, occu_one_19)
occu_model_20  <- occu(~ doy + hr + observer ~ coast_dist, occu_one_20)
occu_model_21 <- occu(~ doy + hr + observer ~ coast_dist, occu_one_21)
occu_model_22 <- occu(~ doy + hr + observer ~ coast_dist, occu_one_22)
occu_model_23 <- occu(~ doy + hr + observer ~ coast_dist, occu_one_23)


occ_fit_19 <- predict(occu_model_19, data = site_covariates_19, type="state",interval="confidence")
...
occ_plot <- ggplot(occ_graph_all %>%
  ...
  labs(title = paste0("Predicted Occupancy change of ",species_choice),
       x = "Year",
...
ggarrange(revi_occ, amre_occ, btnw_occ, nopa_occ, mawa_occ, swth_occ, alfl_occ, yrwa_occ, baww_occ, bhvi_occ,
          blja_occ, chsp_occ, btbw_occ, blbw_occ, gcki_occ, mowa_occ, heth_occ, cswa_occ, eawp_occ, ybsa_occ)

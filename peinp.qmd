...
# Offline work to intersect covariates

nearest_feature <- st_nearest_feature(pei_locs, coast)
...
lk_lc <- tibble(
  land_cover = c("GRS","HAY", "RM", "SM", "SOY", "TRE", "WS", "NA", "PAV", "OTH", "WB", "AL", "DT", "SDW", "PO", "CC", "BAR", "BS", "BF", "NA", "PAS"),
  description = c("Grass", "Hay Field", "Red Maple", "SM", "SOY", "Trees", "White Spruce", "NA", "Paved", "Other", "White Birch",
                  "Alder", "DT", "Sand Dune", "Poplar", "CC", "Bare Soil", "Black Spruce", "Balsam Fir", "Not Available", "PAS"))
...
covs <- distance_to_coast |>
  ...
  mutate(landcover = case_when(landcover %in% c("WS","BS","BF") ~ "Conifer",
                               ...
                               landcover %in% c("SOY","NA","PAV","TRE","HAY","CC","SM","PAS") ~ "Anthro",
...
ggplot(count_annual_mdm, aes(x = year, y = normalized_count)) +
  ...
  annotate("text", x = 2020, y = max(count_annual_mdm$normalized_count), 
...
occu_model_24 <- occu(~ doy2 + hr2 ~ 1, occu_one_24)
...
BANS were detected at r jbm |> select(location) |> tally() distinct locations (`r jbm |> select(location) |> distinct() |> pull()`). Given the low number of detections across the scanned data, it is unlikely that Bank Swallows are occupying areas near these ponds.

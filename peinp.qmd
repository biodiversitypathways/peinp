...
rp <- locs_summary %>%
 mutate(rps = rowSums(across(`2019`:`2024`), na.rm = TRUE)) |>
 filter(rps == 6) |>
 select(Location) |>
 pull()

mdm_spp <- pei_main |>
  filter(location %in% rp) |>
  wt_tidy_species(remove = c("mammal","amphibian","abiotic","insect","unknown"), zerofill = T) |>
  group_by(species_code) |>
  add_tally() |>
  ungroup() |>
...
ggplot(mean_abundance_year, aes(x = year, y = mean_abundance_year)) +
  ...
  labs(
    title = "Trend in Mean Abundance of Medium-Distance Migrants",
    x = "Year",
...
occu_one_24 <- wt_format_occupancy(pei_occu_24, siteCovs = site_covariates_23, species = species_choice)

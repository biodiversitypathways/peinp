...
* Assess the potential of ARUs to detect roosting-associated Bank Swallow activity at pond sites within PEINP, including the timing, call frequency, and configuration effects across monitoring years;
* Offer recommendations for ongoing monitoring approaches, including ARU deployment configurations and scheduling, to contribute to the assessment of ecological integrity in forest and wetland ecosystems;
...
#| results: hide
 ...
 average_deployment <- all_rginecs |>
   group_by(location, year) |>
...
![Bank swallow (C) Environment and Climate Change Canada](./assets/bans.png)

Visual scanning is the concept of visually examining spectrograms in order to find a signal within an audio recording. Visual scanning can be a useful processing method allowing trained users to process recordings much faster than traditional listening. It has been used for detecting different taxa (amphibians: @vis-scan-amphs, mammals: @aru-vs-cams-wolves) with comparable biological metrics, as well as helping to maximize species detection in large acoustic monitoring data sets (@all-the-scans). WildTrax's project settings and dynamic spectrogram settings in the processing interface allow users to upload many recordings, while also allowing frequency-limited or time-limited spectrograms. These changes are easily made by adjusting project settings in WildTrax (see @fig-project-dynamic). Bank Swallow (Riparia riparia) are federally listed as threatened in Canada (see Government of Canada) and provincially. They produce distinctive, broadband vocalizations that are readily identifiable on a spectrogram, making them well-suited to visual scanning workflows.

ARUs were placed near ponds where Bank Swallow are known to forage and roost, as these habitats may support pre-migratory staging aggregations in late summer. Documenting pond use, and in particular the timing and density of vocalizations during dawn and dusk periods, can inform whether ARU deployments are capable of capturing acoustic signatures consistent with roosting behaviour, such as sustained high call-rate events associated with group entry or departure from wetland sites. All targeted recordings were reviewed by a trained observer using visual scanning, with human detections serving as the primary record of Bank Swallow presence.

In 2025, we refined our processing approach using dynamic spectrogram settings (see @fig-project-dynamic), constraining the frequency range to the 95% CI of Bank Swallow vocalizations (2000-7500 Hz) while expanding the time window (1x -> 2x), reducing visual noise and expediting detection. To complement visual scanning and further reduce false negatives, we evaluated two automated classifiers, BirdNET v2.1 (@kahl2021birdnet) and HawkEars v1.0.8 (@huus2025hawkears), against the human-verified detection record. Classifier outputs were overlaid directly on spectrograms in WildTrax, allowing reviewers to cross-reference detections and apply expert judgment where classifier confidence was low. Precision-recall analysis was used to evaluate classifier performance relative to the human record, providing a basis for assessing whether either classifier could serve as a reliable detection tool for this species and behaviour in future seasons.

To address the question of whether ARUs can detect roosting-associated activity, detections were examined by year and site to account for variation in deployment configuration across monitoring seasons. Call counts per recording, rather than binary presence, were used to characterize detection intensity during dawn (04:00–08:00) and dusk (19:00–22:00) survey periods, the windows most likely to capture pre-roost and post-roost vocalizations. Sites monitored in multiple years under different configurations (e.g., Bowley Pond, where one unit was deployed in 2023 and three units in 2025) were examined separately to explore whether detector density affected detection rates or call volume during these periods.

To characterize the temporal distribution of Bank Swallow calling activity, ARU recordings were converted to binary presence/absence records at the individual recording level. Each recording was assigned a day of year and hour of day derived from its timestamp. A generalized additive model (GAM) with a binomial response was fit using the `mgcv` package in R, with smooth terms for day of year (thin plate regression spline, **k** = 10), hour of day (cyclic cubic spline, **k** = 10), and a tensor product interaction term to capture shifting diel patterns across the season. Model parameters were estimated using restricted maximum likelihood (REML). Predicted calling probabilities were generated across a full grid of all hour-by-date combinations within the survey window and visualized as a heatmap, with dawn (04:00–08:00) and dusk (18:00–22:00) survey periods highlighted for reference.
...
bans_projects <- wt_get_projects('ARU') |> filter(grepl('Bank Swallow Monitoring|Prince Edward Island National Park Forest Songbird Monitoring 2021', project)) |> pull(project_id)
...
Automated recognition is a well-known process to help detect rare and elusive species (@knight2019classification. @shonfield2018utility) as well as species that may have a low detectability by collecting large data sets. In order to determine the presence of a few key species-at-risk, we utilized automated recognizers and deployed them across all the recordings in the 2019 data set. We constructed a recognizer for Eastern Wood-Pewee (EAWP) and used three previously constructed Wildlife Acoustics SongScope recognizers for Olive-sided Flycatcher (OSFL), Rusty Blackbird (RUBL) and Canada Warbler (CAWA) to detect for the presence of these species in the 2019 data set. Hits were verified and true positives for presence at each location (recording where the species was first positively detected) were uploaded to WildTrax via the `wt_songscope_tags` function in `wildrtrax` for verification.
...
ggplot(seas_det, aes(x = julian, y = species_common_name, fill = `Nesting habitat`)) + 
...
Bank Swallow vocalizations were detected across multiple survey seasons at two primary pond locations within Prince Edward Island National Park (@tbl-bans-sites). A total of 61 detections were recorded across all years combined, with the majority (61; 100% of multi-unit detections) occurring in 2025. No detections were recorded in 2021 across five sites totalling 202.5 hours of recording and 17.0 hours analyzed. A small number of detections were recorded at Clarke's Pond in 2022 (n = 3; 291.9 hours collected, 28.2 analyzed) and 2024 (n = 2; 164.8 hours collected, 8.5 analyzed), and at Bowley Pond in 2023 (n = 2; 100.2 hours collected, 5.0 analyzed). 

In 2025, three ARUs were deployed simultaneously around Bowley Pond (East, North, and West positions), collectively recording 339.0 hours and yielding 61 detections across 25.6 analyzed hours, representing a substantial increase relative to all prior years combined. Detection rates per analyzed hour were markedly higher in 2025 (approximately 2.4 detections/hour) compared to the single-unit deployments at Clarke's Pond in 2022 (0.11 detections/hour) and 2024 (0.24 detections/hour), and at Bowley Pond in 2023 (0.40 detections/hour).

Predicted calling probability heatmaps generated from the acoustic data revealed distinct diel and seasonal structure in detected vocalizations (@fig-bowley-heat, @fig-all-heat). In the Clarke's Pond dataset, elevated predicted probability was concentrated in a narrow window around early August, primarily between approximately 16:00 and 21:00, consistent with dusk-period activity. A diffuse band of low but non-zero probability was also apparent during the dawn survey window (04:00–08:00 h) across the full survey period. At Bowley Pond in 2025, the predicted calling probability was highest during the dusk window (approximately 18:00–21:00) in the earliest portion of the survey (late July through early August), declining markedly through August. The dawn survey window showed little to no elevated probability at Bowley Pond in 2025.
...
#| include: true
#| code-fold: true
#| label: tbl-bans-sites
#| tbl-cap: "Summary of Bank Swallow ARU monitoring sites within PEINP, 2021–2025. Hours collected reflect total recording effort; hours analyzed reflect processed tasks only. Proportion analyzed is calculated per site per year. BANS detections are the number of unique detections of BANS within a recording under a count-removal framework at the recording level."

# Site name lookup
site_names <- c(
  "PENP-BS-1"   = "Clarke's Pond",
  "PENP-BS-2"   = "Stanhope",
  "PENP-BS-3"   = "Gulf Shore Parkway",
  "PENP-BS-4"   = "North Rustico",
  "PENP-BS-5"   = "Blooming Point",
  "PENP-BS-6"   = "Bowley Pond",
  "PENP-BS-1-1" = "Clarke's Pond",
  "BANS-BOW-E"  = "Bowley Pond (East)",
  "BANS-BOW-N"  = "Bowley Pond (North)",
  "BANS-BOW-W"  = "Bowley Pond (West)")

# Detector count per site per year
detector_config <- tibble::tribble(
  ~location,     ~year, ~n_detectors,
  "PENP-BS-1",   2021,  1,
  "PENP-BS-2",   2021,  1,
  "PENP-BS-3",   2021,  1,
  "PENP-BS-4",   2021,  1,
...
# Hours analyzed (from processed tasks in bans_aru)
tbl_analyzed <- bans_aru |>
  filter(location %in% detector_config$location) |>
  filter(task_is_complete == TRUE) |>
  mutate(year = year(recording_date_time)) |>
  group_by(location, year) |> 
  summarise(hrs_analyzed = round(sum(task_duration) / 3600, 1), .groups = "drop")

tbl_detections <- bans_aru |>
  filter(task_is_complete == TRUE,
         species_code == "BANS") |>
  mutate(year = year(recording_date_time)) |>
  group_by(location, year) |>
  summarise(bans_detections = n(), .groups = "drop")

tbl_dates <- all_recs |>
  mutate(year = year(recording_date_time)) |>
  filter(location %in% bans_locs) |>
  group_by(location, year) |>
  summarise(
    survey_period = paste(
      format(min(recording_date_time), "%b %d"),
      "–",
      format(max(recording_date_time), "%b %d")
    ),
    .groups = "drop"
  )

# Full table
bans_locs <- c("BANS-BOW-E","BANS-BOW-N","BANS-BOW-W",
               "PENP-BS-6","PENP-BS-1","PENP-BS-2","PENP-BS-3",
               "PENP-BS-4","PENP-BS-5","PENP-BS-1-1")

total_bans <- all_recs |>
  mutate(year = year(recording_date_time)) |>
  filter(location %in% bans_locs) |>
  group_by(location, year) |>
  summarise(hrs_collected = round(sum(recording_duration) / 3600, 1), .groups = "drop") |>
  left_join(tbl_analyzed,   by = c("location", "year")) |>
  left_join(tbl_dates,      by = c("location", "year")) |>
  left_join(tbl_detections, by = c("location", "year")) |>
  mutate(
    site_name       = site_names[location],
    pct_analyzed    = round((hrs_analyzed / hrs_collected) * 100, 2),
    hrs_analyzed    = replace_na(hrs_analyzed, 0),
    pct_analyzed    = replace_na(pct_analyzed, 0),
    bans_detections = replace_na(bans_detections, 0)
  ) |>
  arrange(year, location) |>
  select(year, location, site_name, survey_period, hrs_collected, hrs_analyzed, pct_analyzed, bans_detections)
...
total_bans |>
  ...
  cols_label(
    ...
    hrs_collected   = "Hours collected",
    hrs_analyzed    = "Hours analyzed",
    pct_analyzed    = "% analyzed",
    bans_detections = "BANS detections"
  ) |>
  summary_rows(
  groups    = everything(),
  columns   = c(hrs_collected, hrs_analyzed, bans_detections),
...
summary_rows(
  groups    = everything(),
  columns   = pct_analyzed,
  fns       = list(Total = ~round((sum(hrs_analyzed, na.rm = TRUE) / sum(hrs_collected, na.rm = TRUE) * 100),2)),
  formatter = fmt_number,
  decimals  = 2
) |>
  data_color(
    columns        = pct_analyzed,
    target_columns = pct_analyzed,
    method         = "numeric",
    palette        = c("#f7f7f7", "#4A6FA5"),
    domain         = c(0, 100)
  ) |>
  tab_style(
  ...
  data_color(
    columns  = bans_detections,
    method   = "numeric",
    palette  = c("#f7f7f7", "#F4A460"),
    domain   = c(0, max(total_bans$bans_detections, na.rm = TRUE))
  )
```
...
bans_aru |>
  filter(location %in% detector_config$location, species_code == "BANS") |>
  mutate(hour = hour(recording_date_time)) |>
  group_by(hour) |>
  tally() |>
  ungroup()
...
Further breaking down responses at Bowley Pond in 2025, @fig-bans-bowley-day describes the frequency and number of calls detected across the season broken up by dawn and dusk hours of the day throughout the deployment of the recorder. Activity was centered around dusk hours with frequency of detection increasing over the evening hours. 
...
#| eval: true
#| include: true
#| fig-cap: "Bank Swallow detections at Bowley Pond over the season, faceted by detector unit. Bars show total calls detected per day."
#| label: fig-bans-bowley-doy
...
bowley_doy <- bans_aru |>
  filter(task_is_complete == TRUE,
         ...
         species_code == "BANS") |>
  mutate(
    date   = as_date(recording_date_time),
    hour   = hour(recording_date_time),
    period = case_when(
      hour >= 4  & hour <= 8  ~ "Dawn (04:00–08:00)",
      hour >= 18 & hour <= 22 ~ "Dusk (18:00–22:00)",
      TRUE                    ~ "Other"
    )
  )

ggplot(bowley_doy, aes(x = date, fill = period)) +
  geom_histogram(binwidth = 1, colour = "white", linewidth = 0.3) +
  facet_wrap(~ location, ncol = 1) +
  scale_fill_manual(values = c(
    "Dawn (04:00–08:00)" = "#F4A460",
    "Dusk (18:00–22:00)" = "#4A6FA5",
    "Other"              = "grey75"
  )) +
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d") +
  labs(
    x    = NULL,
    y    = "Total calls detected",
    fill = "Survey period"
  ) +
...
Evaluation of the performance of BirdNET and HawkEars on Bank swallow detections can be summarized in the precision-recall curve in @fig-bans-ai. 
...
Finally the predicted heatmap of detection at Bowley Pond (@fig-bowley-heat) and for all Bank swallow monitoring locations (@fig-all-heat). The precision-recall curves demonstrate that BirdNET v2.1 outperforms Hawkears v1.0.8 across most operating points, particularly at moderate to high recall thresholds. BirdNET v2.1 achieves peak precision of approximately 0.53 at a recall of ~0.30, maintaining relatively stable precision (~0.50) across the mid-recall range (0.25–0.60) before declining gradually to ~0.37 at full recall. This plateau suggests the classifier sustains acceptable detection quality even as sensitivity increases. HawkEars v1.0.8 peaks earlier and lower, reaching ~0.45 precision at recall ~0.20, then declines more steeply and consistently. By recall = 1.0, precision drops sharply to ~0.17, roughly half that of BirdNET at the same threshold, indicating a substantially higher false positive rate when the classifier is tuned for maximum sensitivity.
...
#| include: true
#| code-fold: true
#| fig-cap: Heatmap of predicted probability of detection of Bank swallow at Bowley Pond in 2025 with 3 ARU detectors placed around the pond.
#| label: fig-bowley-heat

bans_rec <- bans_aru |>         
  group_by(recording_id, recording_date_time, location) |>
  summarise(
    bans_present = as.integer(any(species_code == "BANS")),
    .groups = "drop"
  ) |>
  mutate(
  ...
  filter(
    # 2025 Bowley Pond survey window
    doy >= yday(as.Date("2025-07-25")),
    doy <= yday(as.Date("2025-08-28"))
  )
...
mod_bans <- gam(
  bans_present ~
    s(doy,  bs = "tp", k = 10) +
    s(hour, bs = "cc", k = 10) +
    ti(doy, hour, bs = c("tp", "cc")),
  family = binomial,
  data   = bans_rec,
  method = "REML"
)

doy_min <- min(bans_rec$doy)
doy_max <- max(bans_rec$doy)
...
newdat <- newdat |>
  mutate(date = as.Date(doy - 1, origin = "2025-01-01"))

# --- 5. Plot ---
ggplot(newdat, aes(date, hour, fill = pred)) +
  geom_raster() +
  scale_fill_viridis_c(
    limits = c(0, 1),
    name   = "Predicted\nProbability",
    option = "magma"       # warm palette suits activity data well
  ) +
  scale_y_continuous(
    breaks = c(0, 4, 8, 12, 16, 18, 20, 22),
    labels = c("00:00", "04:00", "08:00", "12:00",
               "16:00", "18:00", "20:00", "22:00")
  ) +
  # highlight your survey windows as horizontal bands
  annotate("rect", xmin = -Inf, xmax = Inf,
           ymin = 4, ymax = 8,   alpha = 0.1, fill = "orange") +
  annotate("rect", xmin = -Inf, xmax = Inf,
           ymin = 18, ymax = 22, alpha = 0.1, fill = "steelblue") +
  scale_x_date(date_labels = "%b %d", date_breaks = "1 week") +
  labs(
    x     = "Date",
    y     = "Hour of Day",
    title = "Predicted BANS Calling Probability — Bowley Pond 2025",
    subtitle = "Orange band = Dawn survey window  |  Blue band = Dusk survey window"
  ) +
  theme_minimal(base_size = 13) +
...
#| include: true
#| code-fold: true
#| fig-cap: Heatmap of predicted probability of detection of Bank swallow at all monitoring sites.
#| label: fig-all-heat

bans_rec_all <- bans_aru |>          
  group_by(recording_id, recording_date_time, location) |>
  summarise(
    bans_present = as.integer(any(species_code == "BANS")),
    .groups = "drop"
...
# --- 3. Prediction grid ---
doy_min_all <- min(bans_rec_all$doy)
doy_max_all <- max(bans_rec_all$doy)

newdat_all <- expand.grid(
  doy  = doy_min_all:doy_max_all,
  hour = 0:23
)

newdat_all$pred <- predict(
  mod_bans_all,
  newdata = newdat_all,
  type    = "response"
)

# --- 4. Convert doy back to readable dates for x-axis ---
newdat_all <- newdat_all |>
  mutate(date = as.Date(doy))


ggplot(newdat_all, aes(doy, hour, fill = pred)) +
  geom_raster() +
  scale_fill_viridis_c(
    limits = c(0, 1),
    name   = "Predicted\nProbability",
  ...
  scale_x_continuous(
    breaks = c(121, 135, 152, 166, 182, 196, 213, 227, 244),
    labels = c("May 1", "May 15", "Jun 1", "Jun 15",
               "Jul 1", "Jul 15", "Aug 1", "Aug 15", "Sep 1")
  ) +
  annotate("rect", xmin = -Inf, xmax = Inf,
           ymin = 4,  ymax = 8,  alpha = 0.08, fill = "orange") +
  annotate("rect", xmin = -Inf, xmax = Inf,
           ymin = 18, ymax = 22, alpha = 0.08, fill = "steelblue") +
  labs(
    x        = "Date",
    y        = "Hour of Day",
    title    = "Predicted BANS Calling Probability",
    subtitle = "Orange = Dawn  |  Blue = Dusk"
  ) +
  theme_minimal(base_size = 13) +
  theme(panel.grid = element_blank())
...
windows <- tribble(
  ~label,                   ~start, ~end,   ~fill,
  "Spring migration",        110,    145,    "#74C2E1",  # late Apr – late May
  "Breeding",                145,    225,    "#F4A261",  # late May – mid Aug
  "Post-breeding / roost",   225,    255,    "#E76F51",  # mid Aug – mid Sep
  "Fall migration",          255,    290,    "#74C2E1"   # mid Sep – mid Oct
)

fit <- gam(bans_present ~ s(doy, k = 10), 
           data = bans_rec_all, 
           family = binomial)

pred_grid <- tibble(doy = seq(min(bans_rec_all$doy), 
                               max(bans_rec_all$doy), 
                               by = 1))

# Predict outside mutate
preds <- predict(fit, newdata = pred_grid, type = "response", se.fit = TRUE)
...
ggplot() +
  ...
  scale_fill_manual(values = setNames(windows$fill, windows$label),
                    name = NULL) +
  # Raw rug of detections
  geom_rug(data = filter(bans_rec_all, bans_present == 1),
           aes(x = doy), colour = "grey30", alpha = 0.3, sides = "b") +
  # GAM smooth + CI
  geom_ribbon(data = pred_df, aes(x = doy, ymin = lwr, ymax = upr),
  ...
  labs(
    title    = "Bank Swallow — probability of detection over season",
    subtitle = "Atlantic Canada  |  GAM smooth ± 95% CI",
    x = NULL, y = "P(detection)"
  ) +
  theme_bw(base_size = 12) +
...
amphs_only <- pei_main %>%
  ...
  select(location, recording_date_time, species_code, abundance) %>%
  distinct() |>
  left_join(wt_get_species(), by = "species_code")
...
amphs_only %>%
  ...
  scale_fill_viridis_d() +
  labs(title = "Activity per location", x = "Time of Year (Julian Date)", y = "Species") +
  theme_bw()
...
## Community species responses

Considering the recent (2022) natural disturbance effects seen in the Park, species richness and diversity stayed relatively stable. Individuals nesting in conifer dominant stands likely moved to more suitable habitat outside the park, suggesting the potential utility of metapopulation analysis through passive acoustic monitoring outside of the park via citizen science participation. Further analysis may utilize dynamic occupancy models (@mackenzie2009modeling), multi-species multi-season occupancy models (@devarajan2020multi), or community shifts (@oksanen2010canonical) to describe richness and diversity while accounting for decreases in species that may rely on specific habitat structures within the park. Additional geospatial assets could improve the accuracy of modelling apporaches as well. Nevertheless, this study demostrates the efficacy of autonomous recording units (ARUs) as a powerful tool for monitoring climatic and habitat shifts with a relatively low sample size and area coverage. 

## Bank swallows

The temporal concentration of predicted calling probability at both Clarke's Pond and Bowley Pond offers insight into the seasonal and diel activity patterns of Bank Swallows within Prince Edward Island National Park. At both sites, elevated detection probability was associated primarily with the dusk survey window, with peak activity occurring in late July through early August. This timing aligns with the post-breeding period, when adults and newly fledged young aggregate at pre-migratory roost sites before departing south (Garrison 1999, Turner 2004). The rapid decline in predicted calling probability through August at Bowley Pond in 2025 is consistent with this interpretation, likely reflecting the departure of local birds rather than a methodological artefact. Collectively, these patterns suggest that the dusk survey window may be the most acoustically productive period for detecting Bank Swallows at pond roosting sites in this region, and that surveys conducted after mid-August may substantially underestimate local presence.

A central question motivating the 2025 multi-unit deployment at Bowley Pond was whether increasing the number of ARUs around a single waterbody improves detection probability relative to a single-unit design. The 2025 data are suggestive in this regard, but formal inference is constrained by the structure of the available dataset. The spatial distribution of detections across the three Bowley Pond units, East (n = 31), North (n = 14), and West (n = 16), demonstrates that calling activity was not uniformly distributed around the pond perimeter. The eastern unit captured roughly double the detections of either the north or west units, likely reflecting proximity to an active roosting or colony location. Critically, had only a single ARU been deployed at the north or west position, the majority of detections recorded by the east unit would have been missed entirely. This spatial partitioning of detections provides indirect, observational support for the hypothesis that multi-unit arrays improve detection probability at pond sites, by reducing the likelihood that birds active on the far side of a waterbody go undetected.

However, these data cannot support a stronger causal claim. The marked increase in detections in 2025 relative to prior single-unit years, 61 detections versus a maximum of 3 in any previous year, is confounded by several factors that cannot be disentangled post-hoc. These include inter-site differences between Clarke's Pond and Bowley Pond (which may differ in habitat quality, colony size, or roosting suitability), inter-annual variation in Bank Swallow abundance and breeding success, differences in the proportion of hours analyzed across years, and variation in survey timing relative to the post-breeding phenology. Any one of these factors, or their combination, could account for the observed difference in detections independent of ARU number.

Despite these inferential constraints, the 2025 deployment offers a useful proof of concept for future monitoring. The spatial detection asymmetry observed at Bowley Pond underscores a well-recognized limitation of single-point passive acoustic monitoring at open wetland sites: detection probability is strongly influenced by the positional relationship between the recorder and the animal (Acevedo and Villanueva-Rivera 2006, Darras et al. 2018). For colonial or roosting species such as Bank Swallows, where activity may be concentrated along specific pond margins or within emergent vegetation, a single ARU placement is unlikely to characterize the full acoustic environment of the site. A distributed array design, with units positioned at cardinal or opposing points around the waterbody, more plausibly captures the range of spatial variation in calling activity.

To formally quantify the detection benefit of multi-unit over single-unit deployment, future work would benefit from a controlled experimental design, for example, simultaneous deployment of a multi-unit array alongside a designated single-unit reference recorder at the same site and season, allowing direct comparison of detection outcomes under matched conditions. Until such data are available, the 2025 Bowley Pond results are best interpreted as hypothesis-generating: consistent with a detection benefit from multi-unit deployment, but not yet sufficient to quantify its magnitude.

## Monitoring recommendations

As avian species within the Park'ds forested area demonstrate a non-uniform distribution relative to habitat, the following recommendations for optimizing the efficiency and reliability of the ARU program include:

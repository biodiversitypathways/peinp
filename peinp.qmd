...
lk_lc <- tibble(
  land_cover = c("GRS","HAY","RM","SM","SOY","TRE","WS","PAV","OTH","WB","AL","DT","PO","CC","BAR","BS","BF","NA","PAS","SHR","DMW","SDW","MDW","SAW","SSW","OWW","BOW","WSW"),
  description = c("Grass","Hay Field","Red Maple","Soft Maple","Soybean","Trees","White Spruce","Paved","Other","White Birch","Alder","Dead Trees","Poplar","CC","Bare Soil","Black Spruce","Balsam Fir","Not Available","Pasture","Shrub","Deep Marsh","Shrubby Wetland","Meadow Wetland","Saline Wetland","Shrubby Swamp","Open Water Wetland","Bog Wetland","Wooded Swamp")
)
...
datatable(locs_summary |> arrange(Location), 
          options = list(
            searching = TRUE,  
            paging = TRUE,    
            pageLength = 10
...
datatable(task_info, 
  ...
  formatStyle(columns = colnames(task_info), 
              backgroundColor = styleEqual(c("NA"), "lightgray"))  
```
...
Bank Swallows (BANS) were detected at `r jbm |> select(location) |> distinct() |> pull()` locations. Given the low frequency of detections across the surveyed data, along with the cumulative probability modeled, we further processed data by including the number of detection per recording processed and modelled the cumulative detection probability over time (see @fig-bans-dp).
...
#| include: true
#| code-fold: true
#| fig-cap: Performance of two acoustic classifiers on the detectioin of Bank Swallow at the task level. Precision and recall were evaluated based on detections by humans in the entire PEINP EI Monitoring dataset.
#| label: fig-bans-ai

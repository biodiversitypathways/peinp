...
just_bans <- wildrtrax::wt_get_projects(sensor = 'ARU') |>
...
ss_occ_plot_loop <- function(input, species_choice) {
  ...
  site_covs_list <- map(pei_by_year, ~ .x %>%
                          inner_join(covs) %>%
                          ...
                          summarise(across(starts_with(c("Anthro","Deciduous","Open","Conifer")), max), .groups = "drop")
                        )

  # Format occupancy data for each year
  occu_list <- map2(pei_by_year, site_covs_list, ~ wt_format_occupancy(.x, siteCovs = .y, species = species_choice))

  # Scale site and obs covariates
  site_vars <- c("coast_distance","Anthro","Open","Conifer","Deciduous")
  obs_vars  <- c("doy","hr","doy2","hr2")

  occu_list <- map(occu_list, function(o) {
    for (v in site_vars) o@siteCovs[[v]] <- scale(o@siteCovs[[v]])
    for (v in obs_vars)  o@obsCovs[[v]]  <- scale(o@obsCovs[[v]])
    o
  })

  # Fit occupancy models
  models <- map(occu_list, ~ occu(~ doy2 + hr2 ~ Anthro + Open + Deciduous + Conifer, .x))
  names(models) <- unique(pei_occu_all$year)

  # Function to get predictions per model
